...
const FIELD_IDS = {
    ...
    relatedProgrammes: "bceaf1f9-7b59-4305-be1c-5ef5151acf21",
    relatedLearners: "72d59370-e48d-4baa-b69d-7037f7883389",
};
...
const DEFAULT_DUE_DATE_BUSINESS_DAYS = 5;
...
function computeDefaultDueDate(): number {
    return addBusinessDays(new Date(), DEFAULT_DUE_DATE_BUSINESS_DAYS).getTime();
}
...
const LEARNER_ENROLMENT_LIST = "901524848024";
...
async function linkTasks(taskId: string, fieldId: string, relatedTaskIds: string[]) {
    if (relatedTaskIds.length === 0) return;
    await clickupFetch(`/task/${taskId}/field/${fieldId}`, {
          method: "POST",
          body: JSON.stringify({ value: { add: relatedTaskIds, rem: [] } }),
    });
}
...
async function createPipelineForList(
    ...
    const dueDate = setDueDate ? computeDefaultDueDate() : undefined;
    const results = [];
    for (const name of list.templates) {
          results.push(await createTaskWithFields(list.id, name, fields, dueDate));
    }
    return results;
}
...
Deno.serve(async (req: Request) => {
    ...
    try {
      ...
      if (table === "programmes" && type === "INSERT") {
              const tasksByListId: Record<string, any[]> = {};
              for (const list of PROGRAMME_LISTS) {
                        ...
                        tasksByListId[list.id] = await createPipelineForList(list, fields, true);
              }

                     const programmeSetupTasks = tasksByListId[PROGRAMME_SETUP_LIST] || [];
              const learnerEnrolmentTasks = tasksByListId[LEARNER_ENROLMENT_LIST] || [];
              const programmeSetupIds = programmeSetupTasks.map((t) => t.id);
              const learnerEnrolmentIds = learnerEnrolmentTasks.map((t) => t.id);
              for (const t of programmeSetupTasks) {
                        await linkTasks(t.id, FIELD_IDS.relatedLearners, learnerEnrolmentIds);
              }

                     if (record.client_id != null) {
                               const clientTasks = await findTasksForRecord(CLIENT_LIST.id, `clients:${record.client_id}`);
                               for (const t of clientTasks) {
                                           await linkTasks(t.id, FIELD_IDS.relatedProgrammes, programmeSetupIds);
                               }
                     }
      }
