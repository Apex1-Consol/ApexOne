...
{"narratives":{"<field key>":"<text>"},"comments":{"<learner id>":"<text>"},"attendance":[{"learner_id":"<id>","date":"YYYY-MM-DD","status":"P"}],"notes":"<one short sentence on what the file contained and anything you could not use>"}`;
...
Imported file "${fileName}" (text extracted in the browser; ID numbers, emails and phone numbers redacted):
"""
${text}
"""`;
  ...
  for (const [k, v] of Object.entries(out?.narratives || {})) {
    if (fieldKeys.has(k) && typeof v === "string" && v.trim()) narratives[k] = v.trim().slice(0, 4000);
  }
  const comments: Record<string, string> = {};
  for (const [k, v] of Object.entries(out?.comments || {})) {
    if (learnerIds.has(String(k)) && typeof v === "string" && v.trim()) comments[String(k)] = v.trim().slice(0, 1500);
  }
  const seen = new Set<string>();
  const attendance = (Array.isArray(out?.attendance) ? out.attendance : [])
    .map((a: any) => ({ learner_id: String(a?.learner_id ?? ""), date: String(a?.date ?? ""), status: String(a?.status ?? "").toUpperCase().replace("NA", "N/A") }))
    .filter((a: any) => {
