...
type ReqCtx = { fwd: Map<string, string>; back: Map<string, string>; counters: Record<string, number>; refText: string[]; unverified: Set<string>; db?: any };
...
// v22: signed-in users only. No machine secrets are accepted (audit 2026-09-23:
// no cron, trigger or function calls ai-generate). All generator reads go through
// db(), a client carrying the caller's JWT, so table RLS enforces tenant and
// ownership. The service-role client below is used ONLY for ai_usage bookkeeping
// and the admin-only usage_stats email lookup — never for generator data.
...
// The caller-scoped client. Fails closed: there is no fallback to service role.
function db() {
  const c = reqCtx.getStore()?.db;
  if (!c) throw new HttpError(401, "Unauthorized");
  return c;
}

// RLS hides rows the caller may not see, so "missing" and "not yours" look the same.
function notFound(kind: string, id: unknown) {
  return new HttpError(404, `${kind} ${id} not found or you don't have access to it`);
}
...
async function getProgrammesForClient(clientId: number) {
    const { data: byId } = await db().from("programmes").select("*").eq("client_id", clientId);
    if (byId && byId.length) return byId;
    const { data: client } = await db().from("clients").select("client_name").eq("id", clientId).single();
    if (!client?.client_name) return [];
    const { data: byName } = await db().from("programmes").select("*").eq("fields->>Client", client.client_name);
    return byName || [];
}
...
async function generateReportNarrative(clientId: number) {
    const { data: client, error: cErr } = await db()
      ...
      .eq("id", clientId)
      .single();
    if (cErr || !client) throw notFound("Client", clientId);
...
async function generateVisitReport(visitId: number) {
    const { data: visit, error: vErr } = await db()
      ...
      .eq("id", visitId)
      .single();
    if (vErr || !visit) throw notFound("Visit", visitId);
  ...
  if (progId) {
        const { data: prog } = await db().from("programmes").select("fields").eq("id", Number(progId)).single();
...
async function generateDocExtract(assessorId: number) {
    const { data: assessor, error: aErr } = await db()
      .from("assessors")
      .select("*")
      .eq("id", assessorId)
      .single();
    if (aErr || !assessor) throw notFound("Assessor", assessorId);
...
async function generateProgrammeSummary(programmeId: number) {
    const { data: programme, error: pErr } = await db().from("programmes").select("*").eq("id", programmeId).single();
    if (pErr || !programme) throw notFound("Programme", programmeId);
    const progName = programmeLabel(programme);

  const { data: enrolments } = await db().from("enrolments").select("*").eq("programme_id", programmeId);
    const { data: attendance } = await db().from("attendance").select("status").eq("programme_id", programmeId);
    const { data: eisaRows } = await db().from("eisa").select("*").eq("programme_id", programmeId);
    const { data: visits } = await db().from("monitoring_visits").select("*").eq("programme_id", programmeId).order("id", { ascending: false });
    const { data: recs } = await db().from("recommendations").select("*").eq("programme_id", programmeId);
...
async function generateLearnerProgress(learnerId: number) {
    const { data: learner, error: lErr } = await db().from("learners").select("*").eq("id", learnerId).single();
    if (lErr || !learner) throw notFound("Learner", learnerId);
    const name = learnerLabel(learner);

  const { data: enrolments } = await db().from("enrolments").select("*").eq("learner_id", learnerId);
    const progIds = (enrolments || []).map((e: any) => e.programme_id).filter(Boolean);
    const { data: programmes } = progIds.length ? await db().from("programmes").select("id, programme_name, fields").in("id", progIds) : { data: [] };
    const progNameById: Record<number, string> = {};
    for (const p of programmes || []) progNameById[p.id] = programmeLabel(p);

  const { data: attendance } = await db().from("attendance").select("status").eq("learner_id", learnerId);
    const att = attendancePct(attendance || []);

  const { data: poe } = await db().from("poe_checklist").select("*").eq("learner_id", learnerId);
  ...
  const { data: eisaRows } = await db().from("eisa").select("*").eq("learner_id", learnerId);
...
async function generateRiskAssessment(clientId: number) {
  ...
  const risks: any[] = [];
    for (const p of programmes) {
          const { data: visits } = await db().from("monitoring_visits").select("*").eq("programme_id", p.id).order("id", { ascending: false }).limit(1);
          const { data: attendance } = await db().from("attendance").select("status, learner_id").eq("programme_id", p.id);
          const { data: recs } = await db().from("recommendations").select("*").eq("programme_id", p.id);
          const att = attendancePct(attendance || []);
          const openRecs = (recs || []).filter((r: any) => !r.resolved);
          ...
          const { data: lowLearners } = lowIds.length ? await db().from("learners").select("id, first_name, last_name, fields").in("id", lowIds) : { data: [] };
...
async function generateSystemRiskScan() {
    ...
    const [{ data: recs }, { data: expiringRegs }, { data: programmes }] = await Promise.all([
                ...
                db().from("fam_seta_registrations").select("*, assessors(full_name)").not("registration_expiry_date", "is", null).lte("registration_expiry_date", in60),
                db().from("programmes").select("id, programme_name, fields, status, programme_status"),
              ]);
    ...
    const overdueRecs = (recs || []).filter((r: any) => !r.resolved && r.fields?.["Due Date"] && new Date(r.fields["Due Date"]) < today);
      ...
      const { data: visits } = progIds.length
            ? await db().from("monitoring_visits").select("*").in("programme_id", progIds).order("id", { ascending: false })
                  : { data: [] };
...
async function generateRecommendationDraft(visitId: number, kmaDomain: string, score: number) {
    const { data: visit, error: vErr } = await db().from("monitoring_visits").select("*").eq("id", visitId).single();
    if (vErr || !visit) throw notFound("Visit", visitId);
    const f = visit.fields || {};
    let progName = f["Programme Name"]?.[0] || `Programme ${visit.programme_id ?? ""}`;
    if (visit.programme_id) {
          const { data: prog } = await db().from("programmes").select("fields").eq("id", Number(visit.programme_id)).single();
...
async function generateExecutiveSummary() {
    const [{ count: learnerCount }, { count: activeProgCount }, { data: enrolments }, { data: recs }, { data: expiringRegs }] = await Promise.all([
          ...
          db().from("programmes").select("*", { count: "exact", head: true }).eq("status", "active"),
          db().from("enrolments").select("status"),
          db().from("recommendations").select("resolved"),
          db().from("fam_seta_registrations").select("registration_expiry_date").not("registration_expiry_date", "is", null).lte("registration_expiry_date", new Date(Date.now() + 60 * 86400000).toISOString().slice(0, 10)),
        ]);
...
async function generateAttendanceAnalysis(programmeId: number) {
  ...
  const { data: attendance } = await db().from("attendance").select("*").eq("programme_id", programmeId);
    const { data: enrolments } = await db().from("enrolments").select("learner_id").eq("programme_id", programmeId);
    const learnerIds = (enrolments || []).map((e: any) => e.learner_id).filter(Boolean);
    const { data: learners } = learnerIds.length ? await db().from("learners").select("id, first_name, last_name, fields").in("id", learnerIds) : { data: [] };
...
async function generatePoeGapAnalysis(programmeId: number) {
  ...
  const { data: poeRows } = await db().from("poe_checklist").select("*").eq("programme_id", programmeId);
...
async function generateFamComplianceDigest() {
    const { data: assessors } = await db().from("assessors").select("*");
    const { data: allRegs } = await db().from("fam_seta_registrations").select("*");
    const { data: allDocs } = await db().from("fam_documents").select("*").eq("is_current", true);
    const { data: allFollowups } = await db().from("fam_follow_ups").select("*").neq("status", "resolved");
...
// ── Per-user daily caps (signed-in users only) ──
...
async function handle(req: Request): Promise<Response> {
    ...
    // Signed-in users only: the bearer token must be a valid user JWT.
    ...
    if (!verifiedUser) return j({ ok: false, error: "Unauthorized" }, 401);
    reqCtx.getStore()!.db = userClient(token);

    try {
          const body = await req.json();
          // IDs must be positive integers (they are also interpolated into PostgREST filters).
          for (const k of ["client_id", "visit_id", "assessor_id", "programme_id", "learner_id"]) {
                const v = body[k];
                if (v === undefined || v === null || v === "") continue;
                const n = Number(v);
                if (!Number.isSafeInteger(n) || n <= 0) return j({ ok: false, error: `${k} must be a positive integer` }, 400);
                body[k] = n;
          }
          const { type, client_id, visit_id, assessor_id, programme_id, learner_id, kma_domain, score } = body;
          if (!NO_AI_TYPES.has(type)) {
      ...
      let result;
                   switch (type) {
                     ...
                     case "recommendation_draft":
                               if (!visit_id || !/^KMA[1-6]$/.test(String(kma_domain)) || ![1, 2, 3, 4].includes(Number(score))) return j({ ok: false, error: "visit_id, kma_domain (KMA1-KMA6), score (1-4) required" }, 400);
                               result = await generateRecommendationDraft(visit_id, kma_domain, Number(score));
                               break;
                     ...
                     case "usage_stats": {
                               if (verifiedUser.app_metadata?.role !== "admin") return j({ ok: false, error: "usage_stats is admin-only" }, 403);
      ...
      return j({ ok: true, type, ...result, ...(unverified.length ? { fact_check: { unverified_numbers: unverified } } : {}) });
                   ...
                   if (err instanceof HttpError && err.status === 404) {
                         // Log possible probing of records outside the caller's access.
                         const { error: logErr } = await supabase.from("ai_usage").insert({ user_id: verifiedUser.id, type: "access_denied", input_chars: 0, details: { message: err.message.slice(0, 200) } });
                         if (logErr) console.error("ai_usage denial log failed:", logErr.message);
                   }
